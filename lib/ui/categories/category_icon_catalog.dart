...
final Map<String, CategoryIconOption> _categoryIconByKey = {
  for (final option in categoryIconOptions) option.key: option,
};

final Map<int, CategoryIconOption> _categoryIconByCodePoint = {
  for (final option in categoryIconOptions) option.icon.codePoint: option,
};

CategoryIconOption categoryIconOptionForKey(String? key) {
  if (key != null) {
    final option = _categoryIconByKey[key];
    if (option != null) return option;
  }
  return _categoryIconByKey['ellipsis']!;
}

String categoryIconKeyFromLegacy({String? iconKey, int? iconCodePoint}) {
  if (iconKey != null && _categoryIconByKey.containsKey(iconKey)) {
    return iconKey;
  }

  final legacyOption = iconCodePoint == null
      ? null
      : _categoryIconByCodePoint[iconCodePoint];
  return legacyOption?.key ?? 'ellipsis';
}
